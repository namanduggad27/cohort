...
import { transcribeWithSarvam, transcribeWithWhisper, type SarvamTranscriptionResult } from './sarvam.service';
import { extractEntitiesWithHaiku, generateSOAPWithSonnet } from './anthropic.service';
...
import type { Consultation, SOAPNote, RedFlag } from '../models/consultation.model';

// In-memory store for demo
const consultationStore = new Map<string, Consultation>();
...
export async function runFullPipeline(
  ...
  logger.info('Pipeline: Starting full consultation pipeline', {
    requestId,
    consultationId,
    patientId,
  });
  ...
  if (!hasKeys) {
    logger.warn('Pipeline: Running in MOCK fallback mode due to missing API keys', {
      requestId,
      consultationId
    });
  ...
  let sttResult: SarvamTranscriptionResult;
  if (audioBuffer.length < 100) {
    logger.info('Pipeline: Small/simulated audio buffer detected, using mock case transcript', { requestId, patientId });
    ...
    try {
      sttResult = await transcribeWithSarvam(audioBuffer, mimeType, requestId);
    } catch (sarvamErr) {
      logger.warn('Pipeline: Sarvam/Groq STT failed, trying Whisper fallback', {
        requestId,
        error: sarvamErr instanceof Error ? sarvamErr.message : String(sarvamErr),
      });
      try {
        sttResult = await transcribeWithWhisper(audioBuffer, mimeType, requestId);
      } catch (whisperErr) {
        logger.warn('Pipeline: All STT failed (audio unreadable/silent), falling back to case transcript', { requestId });
        const mockCase = getMockConsultation(patientId, doctorId, consultationId, 120);
        sttResult = {
          transcript: mockCase.transcript || '',
          confidence: 0.85,
  ...
  const { entities, usage: haikuUsage } = await extractEntitiesWithHaiku(transcript, requestId);

  // ─── Step 3: RAG Retrieval ───────────────────────────────
  const { context: ragContext, totalTokens: embeddingTokens } = await retrieveRAGContext(
    entities.symptoms,
    entities.drugs,
    requestId
  );
  const ragContextString = buildRAGContextString(ragContext);

  // ─── Step 4: Claude Sonnet — SOAP Synthesis ──────────────
  const { soapNote, usage: sonnetUsage } = await generateSOAPWithSonnet({
    consultationId,
    transcript,
    durationSeconds,
    language,
  ...
  const cost = estimateCost(durationSeconds, haikuUsage, sonnetUsage, embeddingTokens);
...
export function approveConsultation(id: string, doctorId: string): Consultation | null {
...
export function listConsultations(): Consultation[] {
  return Array.from(consultationStore.values()).sort(
